...
type Props = {
    ...
    showDeleted: boolean
    setShowDeletedAction: (v: boolean) => void
        ...
        <div className="h-full flex flex-col">
            ...
            <div className="flex items-center justify-between px-4 pt-4 pb-2 gap-2 shrink-0">
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                    {loading ? '…' : `${leaveTypes.length} type${leaveTypes.length !== 1 ? 's' : ''}`}
                </span>
            ...
            <div className="px-4 pb-2 flex items-center gap-2 shrink-0">
                <Switch
                    id="show-deleted"
                    checked={showDeleted}
                    onCheckedChange={setShowDeletedAction}
                    className="scale-75"
                />
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                {leaveTypes.map((lt) => {
                    ...
                    const isDeleted = !!lt.deletedAt

                    return (
                        <button
                            key={lt.id}
                            onClick={() => onSelectAction(lt.id)}
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-[11px] text-muted-foreground font-mono truncate">{lt.code}</p>
                            </div>
                            <div className="flex flex-col items-end gap-1 shrink-0">
                                <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium
                                    ${lt.isPaid ? 'bg-blue-50 text-blue-600' : 'bg-zinc-100 text-zinc-500'}`}>
